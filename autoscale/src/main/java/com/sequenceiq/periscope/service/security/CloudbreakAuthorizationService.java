...
public class CloudbreakAuthorizationService {
    ...
    public void hasAccess(Long stackId, String userId, String tenant, String permission) {
        if (!cloudbreakClient.withCrn(internalCrnBuilder.getInternalCrnForServiceAsString())
                .autoscaleEndpoint().authorizeForAutoscale(stackId, userId, tenant, permission).isSuccess()) {
